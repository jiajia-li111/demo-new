...
// 尝试引入 Recharts，如果未安装可能会报错，请运行 npm install recharts
...
export default function RealtimeMonitor() {
  ...
  const maxHistoryLength = 20; // 图表保留最近20个点
    ...
    try {
      ...
      if (currentData.heart_rate) {
        setHistoryData(prev => {
          ...
          return newData.slice(-maxHistoryLength); // 保持固定长度
        });
      }
    } catch (err) {
      console.error(err);
      // message.error("获取实时数据失败"); // 频繁报错体验不好，暂注释
    }
  };
